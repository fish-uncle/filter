...
export default class Effect {
	...
	change_color(val) {
		...
		curEntity._ellipse._material.color = new window.Cesium.Color.fromCssColorString(val)
	}
	change_position(p) {
		const cartesian3 = window.Cesium.Cartesian3.fromDegrees(parseFloat(p[0]), parseFloat(p[1]), parseFloat(p[2]))
		const curEntity = this.viewer.entities.getById(this.id)
	...
	add(position, color, maxRadius, duration, isEdit = false) {
		...
		this.viewer.screenSpaceEventHandler.setInputAction(leftDownAction, window.Cesium.ScreenSpaceEventType.LEFT_DOWN)
		this.viewer.screenSpaceEventHandler.setInputAction(leftUpAction, window.Cesium.ScreenSpaceEventType.LEFT_UP)
		this.viewer.screenSpaceEventHandler.setInputAction(mouseMoveAction, window.Cesium.ScreenSpaceEventType.MOUSE_MOVE)
	}
}
