...
export default class MapBuilding extends Factory<MapBuilding> {
	...
	loadRuiAnBuildings(viewer) {
		for (let i = 0; i < this.num; i++) {
			window.Cesium.GeoJsonDataSource.load(`./map/geojson/building/${i}.geojson`).then(geoJSON => {
				viewer.dataSources.add(geoJSON).then(dataSource => {
					for (const entity of dataSource.entities.values) {
						...
						// 材质
						entity.polygon.material = window.Cesium.Color.fromCssColorString('#3895e5').withAlpha(0.8)
						// 描边色
